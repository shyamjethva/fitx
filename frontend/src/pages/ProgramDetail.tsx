...
export default function ProgramDetail() {
  ...
  useEffect(() => {
    const loadProgram = async () => {
      if (!slug) return;
      try {
        const list = await api.getPrograms(FALLBACK_PROGRAMS);
  ...
  if (!program) {
    return (
      <div className="min-h-screen premium-bg force-text-dark flex flex-col items-center justify-center">
        <h2 className="text-3xl font-black mb-4">Program Not Found</h2>
        <Link to="/programs" className="text-[#FFA040] font-bold flex items-center gap-2">
  ...
  return (
    <div className="relative w-full min-h-screen pt-16 pb-32 selection:bg-[#FFA040]/30 premium-bg force-text-dark font-sans">
      {/* Dynamic Background Watermark Label */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none overflow-hidden -z-10">
        <h2 className="text-[12vw] font-black text-[#7A5737]/5 tracking-tighter uppercase leading-none translate-y-[-10%]">
          {program.title}
        </h2>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 pt-12">
        ...
        <button 
          ...
          className="flex items-center gap-3 text-[#7A5737]/70 hover:text-[#7A5737] font-black text-[11px] tracking-[0.2em] uppercase mb-12 group transition-colors"
        >
          <div className="w-8 h-8 rounded-full border border-[#7A5737]/20 flex items-center justify-center group-hover:border-[#7A5737]/50 transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-start">
          ...
          <div className="lg:col-span-7 space-y-10">
            <div className="space-y-6">
              ...
              <p className="text-2xl font-bold tracking-tight max-w-2xl leading-snug italic text-[#7A5737]/80">
                "{program.desc}"
              </p>
            </div>

            {/* Technical Grid Section */}
            <div className="grid grid-cols-3 gap-6 border-y border-[#7A5737]/20 py-8">
              {program.stats.map((stat, index) => (
                <div key={index} className="text-center lg:text-left">
                  <span className="block text-[10px] font-black uppercase tracking-widest mb-1 text-[#7A5737]/60">{stat.label}</span>
            ...
            {/* Description Body */}
            <div className="space-y-6">
              <h3 className="text-xl font-black uppercase tracking-widest">Protocol Architecture</h3>
              <p className="text-lg font-medium leading-relaxed text-[#7A5737]/80">
                {program.longDesc}
              </p>
            </div>

            {/* Checklist Features */}
            <div className="space-y-6 border border-[#7A5737]/20 rounded-[32px] p-8 md:p-10">
              <h4 className="text-xs font-black tracking-[0.3em] uppercase mb-6 text-[#7A5737]/60">Key Deliverables</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {program.highlights.map((highlight, i) => (
                  <div key={i} className="flex items-start gap-4">
                    <CheckCircle2 className="w-5 h-5 shrink-0 mt-1" style={{ color: program.accent }} />
            ...
            <div className="pt-8">
              <button 
                ...
                disabled={isUpdating}
                className="w-full md:w-auto px-16 py-6 rounded-2xl font-black text-xs tracking-[0.3em] uppercase transition-all hover:scale-[1.02] hover:shadow-[0_15px_40px_rgba(0,0,0,0.3)] bg-[#FFA040] text-[#0A0F24] hover:bg-[#FFA040] hover:text-[#0A0F24] disabled:opacity-50 disabled:cursor-not-allowed"
              >
