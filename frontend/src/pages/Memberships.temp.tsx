      ...
      <div className="relative w-full min-h-[420px] md:min-h-0 md:aspect-[25/9] h-auto flex flex-col justify-end">
        ...
        <div className="relative z-10 pb-12 px-6 md:pb-20 md:px-24">
          <motion.div
            ...
            className="max-w-4xl"
          >
            <span className="text-[#FFA040] font-black text-xs md:text-sm tracking-[0.3em] uppercase mb-2 block animate-pulse">
              {subtitle}
            </span>
...
export default function Memberships() {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('ðŸŽ¯ SUCCESS! Request transmitted successfully.');
  ...
  return (
    <div className="relative w-full min-h-screen pt-16 selection:bg-[#FFA040]/30 premium-bg">
      ...
      <div className="flex flex-col">
        ...
        <section className="px-6 md:px-24 py-24 mb-16 bg-gradient-to-br from-white via-orange-50 to-orange-100 rounded-[60px] mx-4 md:mx-8 shadow-2xl border border-orange-200">
            ...
            {packages.map((pkg) => (
              <ScrollReveal key={pkg.id} type="scale">
                <motion.div
                  ...
                  whileHover={{ y: -8 }}
                  className={`membership-card relative p-8 rounded-[32px] border cursor-pointer transition-all duration-500 overflow-hidden h-full flex flex-col bg-white/60 backdrop-blur-xl border-white/80 shadow-lg ${selectedPackage?.id === pkg.id
                    ? 'active scale-[1.02]'
                    : ''
                    }`}
                >
                  {pkg.popular && (
                    <div className="absolute top-6 right-6 bg-[#FFA040] text-white px-4 py-1.5 rounded-full text-[10px] font-black tracking-[0.2em] uppercase shadow-lg">
                      POPULAR
                    </div>
                  )}

                  <div className="mb-8 w-14 h-14 rounded-2xl bg-white border border-gray-100 shadow-md flex items-center justify-center">
                        ...
                        <div className="grid grid-cols-1 gap-6">
                          {selectedPackage.features.map((feature, idx) => (
                            <div key={idx} className="flex items-center gap-5 group">
                              <div className="w-6 h-6 rounded-full bg-white/5 flex items-center justify-center border border-white/10 group-hover:bg-[#FFA040] group-hover:text-[#0A0F24] transition-all">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 gap-6">
                          {selectedPackage.facilities.map((facility, idx) => (
                            <div key={idx} className="flex items-center gap-5 group">
                              <div className="w-6 h-6 rounded-full bg-white/5 flex items-center justify-center border border-white/10 group-hover:border-[#FFA040] transition-all">
                  ...
                  <form className="space-y-5 mt-auto" onSubmit={handleFormSubmit}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <input
                        ...
                        className="dark-input w-full bg-[#121417] border border-white/10 rounded-xl p-5 font-bold text-xs tracking-widest text-white placeholder:text-white/20 focus:border-[#FFA040] transition-all outline-none"
                      />
                      <input
                        type="email"
                        required
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <select
                        ...
                        className="dark-input w-full bg-[#121417] border border-white/10 rounded-xl p-5 font-bold text-xs tracking-widest text-white appearance-none focus:border-[#FFA040] transition-all outline-none cursor-pointer"
                      >
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="dark-input w-full bg-[#121417] border border-white/10 rounded-xl p-5 font-bold text-xs tracking-widest text-white placeholder:text-white/20 focus:border-[#FFA040] transition-all outline-none resize-none"
                    />

                    <button
                      type="submit"
                      disabled={submitting}
                      className="w-full bg-white text-black py-5 rounded-xl font-black text-[11px] tracking-[0.3em] uppercase hover:bg-[#FFA040] hover:text-[#0A0F24] disabled:opacity-50 transition-all mt-4 shadow-xl"
                    >
              ...
              {[
                ...
                <ScrollReveal key={i} type="scale" className="flex-1 flex">
                  <div
                    ...
                    className="info-card bg-[#16191d] p-8 rounded-[24px] border border-white/10 flex items-center gap-6 group cursor-pointer hover:border-[#FFA040]/50 hover:bg-[#1F2328] transition-all w-full h-full"
                  >
                    <div className="w-14 h-14 rounded-2xl bg-[#121417] border border-white/5 flex items-center justify-center flex-shrink-0 transition-all group-hover:border-[#FFA040]">
                      {item.icon}
                    </div>
                    ...
                    <ChevronRight className="w-5 h-5 text-white/20 group-hover:text-[#FFA040] group-hover:translate-x-1 transition-all" />
                  </div>
                </ScrollReveal>
              ))}
            </div>
