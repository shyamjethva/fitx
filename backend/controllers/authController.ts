...
export const googleLogin = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    let payload;
    try {
      // Try fetching userinfo (for access_token from Implicit flow)
      ...
      if (userInfoRes.ok) {
        ...
        // Fallback to idToken verification
      ...
      console.warn("Google token verification failed. Proceeding with decoded token for dev purposes.", err);
      // Fallback decode just for demonstration
    ...
    let userDoc = await User.findOne({ email: lowerEmail }).populate('assignedTrainer');
    ...
    if (!userDoc) {
      ...
      const newUser = new User({
        ...
        role: 'USER',
        assignedTrainer: assignedId,
        appliedProgram: 'FitX Google Member',
        avatar,
        dietPlan: {
          'Mon': { breakfast: 'Oatmeal + Fruit', lunch: 'Rice + Lentils', snack: 'Mixed Nuts', dinner: 'Lean Protein + Broccoli' }
        },
      ...
      await newUser.save();
      userDoc = await User.findById(newUser._id).populate('assignedTrainer');
    }
    ...
    console.error('Google Login server failure:', error);
    res.status(500).json({ error: error.message });
  }
};
