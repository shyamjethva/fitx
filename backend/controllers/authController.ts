...
// ADMIN SEND OTP (email-based via Resend)
export const adminSendOtp = async (req: Request, res: Response): Promise<any> => {
  try {
    const { email } = req.body;
    if (!email) return res.status(400).json({ error: 'Email is required.' });

    // Read admin email from environment (set ADMIN_OTP_EMAIL in .env)
    const adminEmail = (process.env.ADMIN_OTP_EMAIL || process.env.ADMIN_EMAIL || 'admin@fitx.com').toLowerCase().trim();
    ...
    if (resendKey) {
      try {
        ...
        const fromAddr = process.env.OTP_EMAIL_FROM || 'FitX Admin <onboarding@resend.dev>';
        ...
        console.log(`✅ OTP email sent to ${adminEmail} via Resend`);
      } catch (emailErr) {
        console.error('Resend email error (OTP still stored in memory):', emailErr);
        // Don't fail — OTP is still in memory, log it for dev use
        console.log(`⚠️ Fallback — OTP for ${adminEmail}: ${otp}`);
      }
    } else {
      console.log(`⚠️ No RESEND_API_KEY set. OTP for ${adminEmail}: ${otp}`);
    }

    return res.json({
      success: true,
      message: `OTP sent to ${adminEmail}. Check your inbox.`,
      // Do NOT return otpCode in production — email only
    });
  } catch (error: any) {
