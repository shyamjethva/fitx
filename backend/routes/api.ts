...
import { login, register, adminVerifyCode, adminSendOtp, adminVerifyOtp, googleLogin } from '../controllers/authController.ts';
import { getChatHistory, getUsers, postMessage, getTrainerClients, updateClientPlans, updateUserProfile, updateTrainerProfile } from '../controllers/dashboardController.ts';
import { getBlogs, createBlog, updateBlog, deleteBlog } from '../controllers/blogController.ts';
import { getTrainers, createTrainer, updateTrainer, deleteTrainer } from '../controllers/trainerController.ts';
import { getPrograms, createProgram, updateProgram, deleteProgram } from '../controllers/programController.ts';
import { getGalleryItems, createGalleryItem, deleteGalleryItem } from '../controllers/galleryController.ts';
import { getMemberships, getTransformations, createMembership, updateMembership, deleteMembership, createTransformation, updateTransformation, deleteTransformation } from '../controllers/catalogsController.ts';
import { getContacts, createContact, deleteContact } from '../controllers/contactController.ts';
import { getPageHeroes, updatePageHero } from '../controllers/pageHeroController.ts';
...
router.post('/upload', (req, res) => {
  try {
    ...
    const fileUrl = `http://localhost:5008/uploads/${uniqueFilename}`;
